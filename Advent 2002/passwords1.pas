...
BEGIN
   ...
   letter_count:=0;
   for j:=1 to length(current_rule.password) do
   begin
     if current_rule.password[j]=current_rule.target_letter then
       inc(letter_count);
   end;
   if (letter_count >= current_rule.range_low) and (letter_count <= current_rule.range_high) then
   begin
     inc(valid_count);
   end;  
   end;
